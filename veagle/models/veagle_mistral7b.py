...
# Vistion Encoder Model - OWL
# FineTuning Script on the Pretrained model 

'''
Requires Transformer <= 4.36.1 for supporting Mistral 7B 
'''
...
class VeagleMistral(Blip2Base):
    ...
    def __init__(
        ...
        # QFormer
        instruct_blip_model = InstructBlipForConditionalGeneration.from_pretrained(qformer_model_path)
        self.Qformer = instruct_blip_model.qformer
        self.query_tokens = instruct_blip_model.query_tokens

        qformer_params_cache = {
            name: True for name, param in self.Qformer.named_parameters()
        }
        logging.info("Qformer initialised")
        for name, param in instruct_blip_model.named_parameters():
            logging.info("Freeze unused Qformer params")
        ...
        logging.info(f"LLM {llm_model} initialized")

        for name, param in self.llm_model.named_parameters():
            logging.info("Freeze LLM params")
            param.requires_grad = False
        ...
        logging.info("Initialized Vision Projection Layer")
        
        # Overriding Layernorm
